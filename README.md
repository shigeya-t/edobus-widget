...
そのため定常状態で毎回リクエストが発生するのは、到着見込み（`get_guide_message_v2.php`）だけです。
60 秒ごとの更新のたびに、メニューバーアプリから 1 件、ウィジェット拡張から 1 件のリクエストが出ます
（実測でも 3 分間で 8 件 = メニューバーアプリ 4 件 + ウィジェット拡張 4 件、と一致しています）。
...
サーバーが返すメッセージは以下の 7 形式を確認しており、それぞれ区別して表示します。
