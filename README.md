...
時刻表の定刻ではなく、バスロケーションシステムの **GPS 位置情報にもとづく到着見込み** を表示します。
...
3. 通知センターまたはデスクトップで「ウィジェットを編集」から「江戸バス」を追加する
...
WidgetKit にはタイムラインの更新間隔を要求する仕組みがありますが、macOS の更新予算は厳しく、
実測ではアプリを終了した状態で **11 分間に 1 度も更新されませんでした**
（3 分間隔を要求していたにもかかわらず）。
...
定常状態で発生するのは到着見込み（`get_guide_message_v2.php`）だけです。
時刻表は日付単位、祝日一覧は 24 時間、路線・停留所一覧は 1 時間キャッシュしており、
毎回の更新では取得しません。

実測では 3 分間で 8 件（メニューバーアプリ 4 件、ウィジェット拡張 4 件）でした。
更新のたびに、メニューバー用に 1 件と、ウィジェット拡張から 1 件が出ます。
起動直後だけはキャッシュが空のため、路線・停留所・時刻表を含めて数件多くなります。
...
- 「今すぐ更新」は停止中でも 1 回だけ取得します（明示操作のため）
...
- 公式 API でもオープンデータでもありません。提供側の都合で予告なく変更・停止される可能性があります
...
再取得するまで表示は同じ数値のままです。GPS のタイムラグや道路状況により実際とずれることがあります。

### 到着見込みメッセージの扱い

サーバーが返すメッセージは以下の 6 形式を確認しており、それぞれ区別して表示します。
